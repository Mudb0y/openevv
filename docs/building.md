...
What it cannot reach is anything that walks the spine: the moves, the context tests and the string tests want a machine with a language and text in it on both sides, which is a harness of its own. Those are checked by reading, and by the suite where a shipped language calls them.
