...
`-R` is the sample rate, which is not the speed. Nought to five are 8,000, 11,025, 22,050, 16,000, 32,000 and 44,100 hertz, in the order IBM numbered the first four and this port the rest. The default is 11,025.

Above 11,025 the engine goes on running at eight or eleven thousand and each sample is repeated, which is what keeps the voice sounding like Eloquence; `docs/status.md` says why that is better than synthesising at the higher rate and what it does to the spectrum. A number of 8,000 or more is that rate in hertz and is synthesised outright, so `-R 37800` is a rate nobody numbered, and `EVV_UPSAMPLE=none` does the same for the settings so the two can be compared. `tools/rate-compare.py` reads the resulting wave files and says it in numbers:
...
The second is the strongest thing in the file, and it is stronger than any comparison of spectra: a rate reached by holding has to be the engine's own samples, each repeated a whole number of times, and nothing else. That is byte for byte, so it says outright that 22,050 is Eloquence rather than an approximation of it. It is checked across instances rather than within one, because the engine's second utterance is not its first while a fresh instance says the same thing every time. Breaking the copy loop for one sample out of 153,164 fails it and names the sample.
