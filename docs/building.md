...
`test/ipa.sh`, `test/xmltok.sh` and `test/ssml.sh` are the three checks for the SSML layer, and they are the same arrangement as `test/prims.sh`: one file compiled twice, once against our engine and once against IBM's objects, and the script diffs what the two print. `make ipa` and `make -C reference ipa` build the first pair, `make xmltok` and `make -C reference xmltok` the second, `make ssml` and `make -C reference ssml` the third. All three want Wine and IBM's objects.

`test/ipa.c` asks the converters for every code point up to 0x2100 against every symbol that may follow, in six languages, and for the code-set conversions underneath -- 10,934 answers. `test/xmltok.c` drives the XML scanner alone over 46 documents in three modes and prints every token with its text, 961 lines, which is the only way to see the scanner's own state machine rather than what the parser made of it. `test/ssml.c` drives the whole reader through the published filter interface over 176 documents and prints the annotations each turned into.
