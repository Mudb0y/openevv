...
It is a Japanese morphological analyser, not a lookup table. The classes, with how many methods each has:
    ...
    DictMan 26      ConverterInterface 24                InputChar 20 (10 done)
...
The character classes that sweep feeds in are the harness's own copy of `GetCharType`, since `InputChar`'s classifier is in the half of it that is not written yet. That decides only what goes in, and both sides get the same thing, so a mistake there narrows the sweep and cannot hide a difference.
