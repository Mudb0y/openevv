...
One tool note. `objdump` from binutils reuses a local label across sections and then misparses whatever follows it, which turns the middle of a function into nonsense -- `ConvertDakuten` reads as `into` and `add %al,(%eax)` where it is really a compare and a jump. `llvm-objdump -d -r --no-show-raw-insn` does not do that and is what to read these objects with.
