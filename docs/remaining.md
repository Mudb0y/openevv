...
Forty-eight are now written, and `test/prims.sh` is what proves them: the same table of cases compiled twice, once against our engine and once against IBM's objects, comparing the bytes each call leaves behind. `docs/building.md` says what that harness can and cannot reach.

Done, by object. All seven of `assign`: `settvar_l`, `settvar_lng` and `settvar_v`, with `assok`, `noass`, `chkvars` and `chkokass`, the last two being empty in the original and their argument count therefore not recoverable from it. All seven of `stack`: `npush_l`, `ncompare`, `back`, `back_nboa`, `bsclr_pushca`, `bspush_vbot` and `bspop_vbot`. All thirteen of `pointer`, which is the right register's half of the loads and the two ends -- `rpta_loadi`, `rpta_loadl`, `rpta_loadv`, `rpta_mover`, the four `tst` forms, `lpta_loadlng`, and `leftmost` and `rightmost` for both registers. Six of `mem`, which is every arithmetic operation the machine has beyond addition: `vsub`, `vmult`, `vdiv`, `vnegate`, with `divzero` and `vcompareTypeCheck`. Six of `test`: the four bare orderings `testgt`, `testge`, `testlt` and `testle`, with `test_eof` and `test_hasval`. Nine of `optimize`: `if_testle_v_i`, the six `_lng` forms of the fused test-and-branch, `move_lng` and `testneq_tvars`.

Two things that came out of reading rather than being expected. The immediate loads into the right pointer register ask the statement table what kind the *left* register's field is -- 0x44 where 0x54 was meant. It is a slip in the original and it cannot show, because both arms of the switch write the same thing; it is carried here deliberately, and the harness has a case where it could show. And all four `settvar` entry points compile the same body: none of them looks at the width its name announces.

Ninety are left, by object.
...
`access` has fourteen, the stream and context calls: `left_context`, `right_context`, `allow_left_ctxt`, `allow_right_ctxt`, `init_stream`, `num_fields_in_stream`, `first_fieldval`, `next_fieldval`, `valid_prefix`, `valid_prefix_char`, `project_sync`, `merge_sync`, `divide_time` and `unique_value`.
