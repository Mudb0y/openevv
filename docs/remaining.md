...
`delta` holds most of what is left of the generate family, twelve calls: `generate`, `gen_copy`, `gencur_framedur`, `gencur_params`, `gencur_timestm`, `gendef_framedur`, `gendef_params`, `gendef_timestm`, `merge`, `insert_2pt`, `mark_l` and `mark_lng`. Four of `gener`'s five are done and the fifth, `vgenerate`, is three lines standing on `vgen` -- a thousand lines of disassembly in `dttime.obj` and the largest single function left anywhere in the machine. That pair is the piece to take next if the acoustic side is wanted, and the cells the four collectors fill are named in `delta.h` now, which was most of the work.
