...
Forty-eight are left, by object.
...
`delta` has three left, and each is a short guard standing on something longer that is also missing: `generate` on `vgen`, `merge` on `vmergable`, and `insert_2pt` on `visnonseq`. `vgen` is a thousand lines of disassembly in `dttime.obj` and the largest single function left anywhere in the machine; `vmergable` and `visnonseq` are about four hundred each in `ddelta.obj`. Those three pairs are what stands between this and the whole generate path, and the cells they fill are named in `delta.h` now, which was most of the work.

`ddelta` has eight left: `CLRNONSEQ`, `TVFLDS`, `mapsyncs`, `vclrctxt`, `vredoctxt`, `streamName`, and the two long ones `visnonseq` and `vmergable` that `delta`'s guards want. `access` has one left, `merge_sync`, with the static `safe_mergable` under it -- six hundred bytes of walking a node's marks either side and asking `visleft` and `visright` whether the two may be joined, and the one call in that object that wants a spine to be tested on. `heap` has `peekDeltaStackStart`, `peekDeltaStackNext`, `resetDeltaStack` and the two heap-object allocators. `dttime` has five: `vgen`, and the four of the `val_expr` chain that `src/eci_deltamisc.c` says why it left out. `ctxt` has `vctxtinit`, `extra` has `prt_tvar` and `set_saved_ptrs`, and `optbl` has `ins_rdtoks`.
