...
#: reader and nothing written down. That is the silence this guards: speech
#: stops, and stays stopped until something else asks for silence and stops
#: the player as a side effect.
#:
...
class Engine:
	...
	def _watch(self):
		...
		with nothing in the log, and it lasts until something else asks for
		silence and stops the player as a side effect -- which is why it looks
		like speech coming back when you leave the window.
