...
def stall_checks():
    ...
    with nothing said and nothing written down. That is a silence that lasts
    until something else happens to ask for silence and stops the player as a
    side effect, which is why it looks like speech returning when the reader
    leaves the window.
    """
    import threading
    import time
...
def control_checks():
    ...
    # Queued but not yet run, since the thread is waiting on the library.
    ...
    check("and keeps the settings", len([k for k, _ in left if k == mod.CONTROL]), 1)
    engine.close()
