...
def store_name(obj, secname, number):
    ...
    return "evv_%s%s_%d" % (re.sub(r"[^A-Za-z0-9_]", "_", obj[:-4]),
                            re.sub(r"[^A-Za-z0-9_]", "_", secname), number)


def write_consts(e, where, out_c):
...
def write_c(e, where, out_c, out_h, out_syms, stores, names):
...
def write_trace(e, where, out_c, out_dir, known, swapped):
    ...
    with open(out_c, "w") as f:
        f.write("/* Generated by tools/delta-emit.py. Do not edit. */\n\n"
                ...
                "static unsigned tally(const unsigned char *p, int n,\n"
                ...
                "        int j;\n\n"
                "        memcpy(&w, p + i, 4);\n"
                "        /* Two runs of the engine put it at different\n"
                "           addresses, so anything that looks like one is\n"
                "           counted by where it points rather than by what\n"
                "           it is, and anything further afield not at all. */\n"
                "        if (d != 0 && w >= (unsigned)(size_t)d\n"
                "            && w < (unsigned)(size_t)d + 0x2000u)\n"
                "            w = 0x10000000u + (w - (unsigned)(size_t)d);\n"
                "        else if (v != 0 && w >= (unsigned)(size_t)v\n"
                "                 && w < (unsigned)(size_t)v + 0x2000u)\n"
                "            w = 0x20000000u + (w - (unsigned)(size_t)v);\n"
                "        else if (w >= 0x100000u)\n"
                "            w = 0x30000000u;\n"
                "        for (j = 0; j < 4; j++)\n"
                ...
                "            (size_t)(unsigned)a[0];\n"
                "        const unsigned char *v =\n"
                "            *(const unsigned char *const *)(d + 0x68);\n"
                "        const unsigned char *k =\n"
                "            *(const unsigned char *const *)(d + 0x6c);\n\n"
                "        fprintf(stderr, \"  [%08x %08x %08x]\\n\",\n"
                "                tally(d, 0x1000, d, v),\n"
                "                v ? tally(v, 0x1200, d, v) : 0,\n"
                "                k ? tally(k, 0x800, d, v) : 0);\n"
                "    }\n"
