...
def main():
    ...
    print("engine/disassemble: reading %s" % obj)
    if not os.path.exists(obj):
        print("engine/disassemble: error: object not found; run tools/sdk/extract.sh first", file=sys.stderr)
        return 1
    ...
    print("engine/disassemble: %d functions, %d data tables, %d parameters, %d undefined symbols"
          % (len(funcs), len(tables), len(params), len(undefined)))
    print("engine/disassemble: wrote %s" % base)
    return 0
