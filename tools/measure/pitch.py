...
def read_wav(path):
    ...
    if raw[:4] != b"RIFF" or raw[8:12] != b"WAVE":
        raise SystemExit("measure/measure/pitch: %s is not a wav" % path)
    fmt = None
    data = None
    at = 12
    while at + 8 <= len(raw):
    ...
    if fmt is None or data is None:
        raise SystemExit("measure/measure/pitch: %s has no format or no samples" % path)
    ...
    if tag == 1 and bits == 16:
        ...
        raise SystemExit("measure/measure/pitch: %s is format %d at %d bits, which this does"
...
def survey(tsv, where):
    ...
    if not done:
        print("measure/measure/pitch: nothing in %s to measure" % where)
        return False
