...
def add_phoneme(tag, name, args):
    ...
    st = statement(tag)
    if name in st["names"]:
        raise SystemExit("module/phonemes: %s already has %r, as code %d"
                         % (tag, name, st["names"].index(name)))
    stride = st["stride"]
    if len(st["variants"]) != len(st["names"]) * stride:
        raise SystemExit("module/phonemes: %d names and %d bytes of records is"
                         " not one record each"
                         % (len(st["names"]), len(st["variants"])))
    rec = bytearray(stride)
    for a in args:
        if "=" not in a:
            raise SystemExit("module/phonemes: %r is not field=value" % a)
        k, v = a.split("=", 1)
        if k not in st["record"]:
            raise SystemExit("module/phonemes: a record has no %r; it has %s"
                             % (k, ", ".join(st["record"])))
        table = st["values"].get(k, [])
        if v not in table:
            raise SystemExit("module/phonemes: %s has no value called %r; it has"
                             " %s" % (k, v, ", ".join(table)))
...
def set_record(tag, phoneme, args):
    ...
    if phoneme not in st["names"]:
        raise SystemExit("module/phonemes: %s has no phoneme called %r"
                         % (tag, phoneme))
    code = st["names"].index(phoneme)
    stride = st["stride"]
    if len(st["record"]) != stride:
        raise SystemExit("module/phonemes: the statement says a record is %d"
                         " bytes and names %d fields for it"
                         % (stride, len(st["record"])))
    at = code * stride
    if at + stride > len(st["variants"]):
        raise SystemExit("module/phonemes: %s has no record" % phoneme)
    ...
    for a in args:
        ...
        table = st["values"].get(k, [])
        if v not in table:
            raise SystemExit("module/phonemes: %s has no value called %r; it"
