...
def emit(m, out_c, tag):
    ...
    with open(out_c, "w") as f:
        f.write("/* Generated by tools/module/sets.py. Do not edit.\n"
                "\n"
                ...
                "   tools/module/dict.py out of lang/enus/enus.dict, which is\n"
...
# tools/module/dict.py out of the words, so IBM's objects hold what the
...
def numbers(s, name):
    m = re.search(r"static const uint8_t %s\[\] = \{(.*?)\};" % name, s,
                  re.S)
    if m is None:
        raise SystemExit("module/sets: no %s in there" % name)
...
def model_from_c(path):
    ...
    for which, name in (("sets", "setent"), ("acts", "actent")):
        body = re.search(r"static const uint8_t \*const %s_all\[\] = \{"
                         r"(.*?)\};" % name, s, re.S)
        if body is None:
            raise SystemExit("module/sets: no %s_all in there" % name)
...
# through tools/module/dict.py, which lays the dictionary's three arrays down
...
#
# Written by tools/module/sets.py. The words themselves are not in here: they
# are in %(tag)s.dict.
"""
