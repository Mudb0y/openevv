...
def section_data(obj, want):
    ...
    if want not in where:
        raise SystemExit("module/settings: %s is not in %s" % (want, obj))
...
def library(obj):
    ...
    if len(whole) == 1:
        ...
        if len(family) != 1 or not dialects:
            raise SystemExit("module/settings: getLibraryName asks a question this"
    ...
    if at < 0 or raw.find(LIBNAME_PREFIX, at + 1) >= 0:
        raise SystemExit("module/settings: the library name is not in there once")
...
def language_of(blob):
    ...
    raise SystemExit("module/settings: no section names a language")
...
def main(argv):
    ...
    if at != 0:
        raise SystemExit("module/settings: the blob does not start its section")
    size_at = syms[SIZE][1]
    size = int.from_bytes(data[size_at:size_at + 4], "little")
    if not 0 < size <= size_at:
        raise SystemExit("module/settings: %d is not a size this blob could have"
                         % size)
    blob = data[:size]
    if blob[-1] != 0xff and 0xff not in blob[-2:]:
        raise SystemExit("module/settings: the blob does not end the way one does")
    ...
    if declared not in packed:
        raise SystemExit("module/settings: the code answers for %s and the sections"
                         " name 0x%x"
...
def emit(tag, blob, declared, name, out):
    ...
    with open(out, "w") as f:
        f.write("/* The engine's settings, built into the image.\n"
                ...
                " tools/module/settings.py rather than\n"
    ...
    while i < len(line):
        ...
        if c == "\\" and i + 1 < len(line):
            ...
            if nxt == "n":
                ...
                raise SystemExit("module/settings: \\%s is not an escape" % nxt)
            i += 2
            continue
...
def write_text(tag, blob, name, path):
    ...
    out = ["# The engine's settings for %s, as text." % tag,
           ...
           "# language declares.",
           "#",
           "# Written by tools/module/settings.py.",
           "",
           "library %s" % name,
           ""]
...
def read_text(path):
    ...
    if name is None:
        raise SystemExit("module/settings: the text does not say the library name")
