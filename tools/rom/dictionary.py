...
def blobs(where):
    ...
    for f in sorted(os.listdir(where)):
        ...
        if not data:
            raise SystemExit("rom/dictionary: %s names blobs and holds no data" % f)
        for i, (name, at) in enumerate(syms):
            end = syms[i + 1][1] if i + 1 < len(syms) else len(data)
            if at >= end or end > len(data):
                raise SystemExit("rom/dictionary: %s in %s runs from %d to %d of %d"
...
def arrays(obj):
    ...
    def flush():
        ...
        if not a or not b:
            raise SystemExit("rom/dictionary: a store names %r and %r" % tuple(seen))
        if at % 4:
            raise SystemExit("rom/dictionary: an index at byte %d is not a slot" % at)
...
def emit(data, table, out, tag):
    ...
    with open(out, "w") as f:
        f.write("/* The Japanese romanizer's static dictionary.\n"
                ...
                " * byte for byte out of IBM's objects by tools/rom/dictionary.py"
                " rather than retyped:\n"
...
def main(argv):
    ...
    data = blobs(where)
    if not data:
        raise SystemExit("rom/dictionary: no static dictionary in %s" % where)
    table = arrays(os.path.join(where, "jpnsdict.obj"))
    if not table:
        raise SystemExit("rom/dictionary: Initialize said nothing")

    missing = sorted({n for slots in table.values() for n in slots.values()}
                     - set(data))
    if missing:
        raise SystemExit("rom/dictionary: %d blobs named and not found, first %s"
