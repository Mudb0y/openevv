...
echo "sdk/sdk/extract-langs: reading $LIBDIR"
[ -d "$LIBDIR" ] || { echo "sdk/sdk/extract-langs: no such directory" >&2; exit 1; }

for tag in $LANGS; do
  lib="$LIBDIR/eci$tag.lib"
  if [ ! -f "$lib" ]; then
    echo "sdk/sdk/extract-langs: $tag has no library"
    continue
  fi
  out="$ROOT/analysis/$tag"
  mkdir -p "$out"
  (cd "$out" && rm -f ./*.obj && llvm-ar x "$lib")
  echo "sdk/sdk/extract-langs: $tag  $(ls "$out"/*.obj | wc -l) objects"
done
