...
echo "sdk/sdk/extract: reading $LIBDIR"

if [ ! -d "$LIBDIR" ]; then
  echo "sdk/sdk/extract: error: SDK library directory not found: $LIBDIR" >&2
  exit 1
fi
...
for f in clsyn.obj runklatt.obj; do
  echo "sdk/sdk/extract: $f $(stat -c %s "$f") bytes"
done
...
echo "sdk/sdk/extract: $(ls "$ROOT/analysis/enus"/*.obj | wc -l) objects from ecienus.lib"
...
rm -f test-glob.obj
echo "sdk/sdk/extract: renamed $(wc -l < rename.txt) symbols, widened $(wc -l < globalize.txt) statics"
...
echo "sdk/sdk/extract: prefixed $(wc -l < delta-rename.txt) Delta symbols into analysis/delta-ibm"
